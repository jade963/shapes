...
/**
 * class for all interactive objects
 * @param {number} this.xStart = x=coordinate for where the mouse initially went down
 * @param {number} this.yStart = y-coordinate for where the mouse initially went down
 * @param {number} this.xMouse = x-coordinate for where the mouse currently is
 * @param {number} this.yMouse = y-coordinate for where the mouse currently is
 * @param {boolean} this.mouseIsDown = whether or not the mouse is currently down
 */
...
class InteractiveObject{
    ...
    // finds out if the mouse is within the object's boundaries - returns true or false
...
/**
 * @param {number} this.x = x-value of button
 * @param {number} this.y = y-value of button
 * @param {number} this.w = width of button
 * @param {number} this.h = height of button
 * @param {number} this.lineW =  width of stroke around button
 * @param {number} this.hoverLine = width of stroke around button when being hovered over
 * @param {string} this.fillC = fill colour of button
 * @param {string} this.strokeC = stroke colour of button
 * @param {string} this.hoverC = stroke colour of button when being hovered over
 * @param {string} this.selectedC = stroke colour of button when selected
 * @param {string} this.text = text on button
 * @param {string} this.textC = colour of text
 * @param {boolean} this.inBounds checks if the mouse is inside the button when it clicks
 * @param {string} this.name = name of class (Button in this case but will change as this code is extended in similar
 * classes)
 * @param {number} this.xStart = initial x-coordinate for location where the mouse goes down
 * @param {number} this.yStart = initial y-coordinate for location where the mouse goes down
 * @param {boolean} this.amIDrawing = ensures the user cannot edit the button if they did not click inside the console
 *  area
 * */
...
/**
 * class that works as an on/off switch for certain functionalities
 * @param {string} this.fullText = everything the button says as seen by the user
 * @param {string} this.type = the first word on the button, which shows the user what they are interacting with
 * @param {string} this.status = the second word on the button, "on" or "off"
 * @param {boolean} this.selected = whatever or not button is selected (all togglers start off selected/"on")
 */
...
/**
 * Class for buttons with sub_buttons
 * @param {string} this.type = either "parent" or "child" (the two categories of multi_Button)
 * @param {string} this.shape = the name of the relevant "parent" shape that all multi_Buttons in a group will
 * respond to
 */
class Multi_Button extends Button {
    constructor(x, y, w, h, lineW, hoverLine, fillC, strokeC, hoverC, selectedC, text, textC, type, shape, xStart,
                yStart, inBounds, amIDrawing) {
        super(x, y, w, h, lineW, hoverLine, fillC, strokeC, hoverC, selectedC, text, textC, xStart, yStart, inBounds,
            amIDrawing);
    ...
    mClick() {
        ...
        if (this.inBounds && this.amIDrawing === false) {
            ...
            let star_list = [BStar,B4,B5,B6,B7,B8];
            ...
            let button_list = [arrow_list, ellipse_list,star_list];
...
/* class for a group of buttons unrelated to the shape Buttons - these are used to determine whether the user is editing
 stroke or fill colour (as there is only one colour palette)
*/
class Colour_Button extends Button{
    constructor(x, y, w, h, lineW, hoverLine, fillC, strokeC, hoverC, selectedC,text, textC, xStart, yStart, inBounds,
                amIDrawing){
        super(x, y, w, h, lineW, hoverLine, fillC, strokeC, hoverC,selectedC,text, textC, xStart, yStart, inBounds,
            amIDrawing);
...
// class for a button that deselects as soon as the user clicks anywhere else (used for undo/redo/clear currently)
...
// class for a group of buttons unrelated to the shape Buttons - these are used to determine stroke width
...
// class for a group of buttons unrelated to the shape Buttons - these are used to determine the dash
...
// class for drawing a grid (used on canvas background)
class Grid{
 ...
 * @param {number} this.x = xStart
 * @param {number} this.y = yStart
 * @param {number} this.w = width of shape
 * @param {number} this.h = height of shape
 * @param {boolean} this.fill = whether the shape should be filled or transparent
 * @param {boolean} this.stroke = whether the shape should be stroked or edgeless
 * @param {string} this.fillC = fill colour
 * @param {string} this.strokeC = stroke colour
 * @param {number} this.lineWidth = line width
 * @param {string} this.dash = whether or not the shape's outline is dashed
 */
class Rectangle{
 ...
 * @param {number} this.x = xStart
 * @param {number} this.y = yStart
 * @param {number} this.x2 = xMouse
 * @param {number} this.y2 = yMouse
 * @param {number} this.radiusX = horizontal radius of ellipse
 * @param {number} this.radiusY = vertical radius of ellipse
 * @param {number} this.rotation = rotation of ellipse
 * @param {number} this.startAngle = starting angle of elliptical arc
 * @param {number} this.endAngle = ending angle of elliptical arc
 * @param {string} this.fill = whether the shape should be filled or transparent
 * @param {string} this.stroke = whether the shape should be stroked or edgeless
 * @param {string} this.fillC = fill colour
 * @param {string} this.strokeC = stroke colour
 * @param {number} this.lineWidth = line width
 * @param {string} this.dash = whether the shape's outline is dashed
 */class Ellipse{
    ...
    update(){
        this.basicEllipse(this.x, this.y, this.x2,this.y2, this.radiusX, this.radiusY, this.rotation,this.startAngle,
            this.endAngle,
 ...
 * @param {number} this.x1 = xStart
 * @param {number} this.y1 = yStart
 * @param {number} this.x2 = xMouse
 * @param {number} this.y2 = yMouse
 * @param {string} this.strokeC = stroke colour
 * @param {number} this.strokeW = line width
 * @param {string} this.dash = whether or not the shape's outline is dashed
 */
class Line{
 ...
 * @param {number} this.x2 = xMouse
 * @param {number} this.y2 = yMouse
 * @param {number} this.x3 = third point of triangle (x-coordinate)
 * @param {number} this.y3 = third point of triangle (y-coordinate)
 * @param {boolean} this.fill = whether the shape should be filled or transparent
 * @param {boolean} this.stroke = whether the shape should be stroked or edgeless
 * @param {string} this.fillC = fill colour
 * @param {string} this.strokeC = stroke colour
...
class Triangle{
    ...
    update(){
        this.basicTriangle(this.x1,this.y1,this.x2,this.y2,this.x3,this.y3,this.fill,this.stroke,this.fillC,
            this.strokeC,
...
/** class for creating arrow objects
 * @param {number} this.x1 = xStart
 * @param {number} this.y1 = yStart
 * @param {number} this.x2 = xMouse
 * @param {number} this.y2 = yMouse
 ...
 */
class Arrow{
    constructor(x1,y1,x2,y2,fill,stroke,fillC,strokeC,lineWidth,dash,type,stickC){
        this.x1 = x1;
        this.y1 = y1;
        this.x2 = x2;
        this.y2 = y2;
    ...
    update(){
        this.basicArrow(this.x1,this.y1,this.x2,this.y2,this.fill,this.stroke,this.fillC,this.strokeC,this.lineWidth,
            this.dash,this.type,this.stickC);
    }
}
Arrow.prototype.basicArrow = basicArrow;


/** class for creating star objects
 * @param {number} this.outer_radius = radius of star's outer points
 * @param {number} this.inset = radius of star's inner points
 * @param {number} this.n = number of points
 * @param {number} this.x1 = xStart
 * @param {number} this.y1 = yStart
 * @param {boolean} this.fill = whether the shape should be filled or transparent
 * @param {boolean} this.stroke = whether the shape should be stroked or edgeless
 ...
 */
class Star{
    constructor(outer_radius,n,x1,y1,fill,stroke,fillC,strokeC,lineWidth,dash){
    ...
    update(){
        this.basicStar(this.outer_radius,this.inset,this.n,this.x1,this.y1,this.fill,this.stroke,this.fillC,
            this.strokeC,this.lineWidth,this.dash);
    }
}
Star.prototype.basicStar = basicStar;
...
Xa,Ya through to Xg,Yg represent points of the arrow to be drawn (see planning)
 */
function basicArrow(x1,y1,x2,y2,fill,stroke,fillC,strokeC,lineWidth,dash,type,stickC){
    let w = x2 - x1;
    let h = y2 - y1;
    let Xa = x1;
    let Ya = y1 + h/3;
...
function basicEllipse(x, y, x2,y2, radiusX, radiusY, rotation, startAngle, endAngle, fill, stroke, fillC, strokeC,
                      lineWidth,dash,type){
    ...
    if (type === "Full"){
        ...
        // this section calculates what direction and rotation the quarter_ellipse should take while being drawn
        if (y2 > y && x < x2){
...
function tempEllipse(x,y,x2,y2,radiusX,radiusY,rotation,startAngle,endAngle,type){
    ...
    if (type === "Full"){
        ...
        // the half-ellipse can flip over the horizontal axis
        if (y2 > y){
            rotation = 0;
        }
        else if (y2 < y){
...
function basicStar(outer_radius,inset,n,x1,y1,fill,stroke,fillC,strokeC,lineWidth,dash){
    if (dash === "small"){
        ctx.setLineDash([4,16]);
        ctx.lineDashOffset = 4;
    }
    ...
    ctx.save();
    ctx.translate(x1,y1);
    ctx.moveTo(0,0-outer_radius);
    // rotates around the axis and draws from point to point
    for (let i = 0; i < n; i++){
        ctx.rotate(Math.PI / n);
        ctx.lineTo(0,0-(outer_radius * inset));
        ctx.rotate(Math.PI / n);
        ctx.lineTo(0,0-outer_radius);
    }
    if (fill === true){
        ctx.fill();
    }
    /* another line is drawn here to close the star path cleanly (as otherwise the ctx.closePath() creates an awkward
    junction */
    ctx.rotate(Math.PI / n);
    ctx.lineTo(0,0-(outer_radius * inset));
    ctx.restore();
...
// function for drawing a "guide star"
function tempStar(outer_radius,n,x,y){
    ctx.setLineDash([]);
    ctx.lineWidth = 2;
    ctx.strokeStyle = "rgb(0,0,0)";
    ctx.beginPath();
    ...
    for (let i = 0; i < n; i++){
        ctx.rotate(Math.PI / n);
        ctx.lineTo(0,0-(outer_radius * 0.5));
        ctx.rotate(Math.PI / n);
        ctx.lineTo(0,0-outer_radius);
    }
    ctx.restore();
